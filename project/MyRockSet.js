...
export class MyRockSet extends CGFobject{
    constructor(scene, numberOfRocks){
        ...
        for (var i = 0; i < numberOfRocks; ++i) {
            var position = [Math.random()*50 - 25, 1.6, Math.random()*50 - 25];
    ...
    getNearestRock(x, z) {
        ...
        for(var i = 0; i < this.rocks.length; ++i){
            if (this.rocks[i][2]) {
                var dist = this.distance(this.rocks[i][1][0], this.rocks[i][1][2], x, z);
    ...
    display(){
        for(var i = 0; i < this.numberOfRocks; ++i){
            if(this.rocks[i][2]){
                ...
                this.scene.translate(...this.rocks[i][1]); // TODO This is a bit ugly
